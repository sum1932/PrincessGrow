...
2D 비주얼노벨 감성의 육성 시뮬레이션 게임입니다. Unity로 플레이 화면을 구현하면서도 게임의 핵심 규칙은 순수 C# Core로 분리했고, 별도 콘솔 시뮬레이터에서 랜덤/LLM 전략으로 자동 플레이를 돌려 밸런싱과 이벤트 흐름을 검증할 수 있도록 설계했습니다.
...
- 턴 진행, 스탯, 경제, 이벤트, 엔딩 판정 등 Core 게임 로직 설계 및 구현
- Unity 의존성을 제거한 순수 C# 도메인/서비스 계층 구성
- Core와 Unity UI를 연결하는 Presenter/Input 어댑터 설계
- CSV/JSON Repository 기반의 콘텐츠 데이터 파이프라인 구현
- 자동 플레이 시뮬레이터 및 LLM 플레이어 전략 구현
...
Unity에서 바로 실행되는 게임이지만 핵심 규칙은 `MonoBehaviour`나 Unity API에 직접 의존하지 않도록 분리했습니다. 덕분에 Unity Editor를 열지 않고도 콘솔 프로젝트에서 턴 진행, 스탯 변화, 이벤트 발생, 엔딩 판정을 검증할 수 있습니다.
...
Core 로직은 화면 표시나 입력 장치를 알지 못하고, `IGamePresenter`와 `IGameInput` 같은 인터페이스만 통해 외부와 연결됩니다. Unity 쪽 구현체는 Core 상태를 UI View와 UnityEvent로 변환하는 얇은 어댑터 역할만 담당합니다.
...
`ISimulationStrategy`를 통해 랜덤 전략, 인터랙티브 전략, LLM 전략을 교체할 수 있도록 만들었습니다. 한 판의 전체 턴 흐름을 자동 실행하면서 엔딩 분포, 스탯 변화, 경제 밸런스, 이벤트 선택 흐름을 빠르게 확인할 수 있습니다.
...
- Unity 프로젝트에서 게임 로직과 View 계층을 분리하는 설계 능력
- CSV/JSON 기반 데이터 파이프라인을 통한 콘텐츠 확장성
- 자동화 시뮬레이션으로 밸런싱과 QA를 보조하는 개발 방식
- LLM을 게임 플레이 테스트에 연결하는 실험적 구현 경험
- 포트폴리오에 코드 근거를 함께 제시할 수 있는 구조화된 저장소 관리
