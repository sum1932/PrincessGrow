...
namespace DessertKingdom.Core.Services
{
    public interface IEventManager
    {
        ...
        void ProcessEventChoice(GameEvent gameEvent, EventChoice choice);
    }
    ...
    public class EventManager : IEventManager
    {
        ...
        public void ProcessEventChoice(GameEvent gameEvent, EventChoice choice)
        {
            ...
            {
                _state.Character.ModifyStat(effect.Key, effect.Value);
            }
            
            // 호감도 효과 적용
            foreach (var favor in choice.FavorEffects)
            {
                var npc = _state.GetNPC(favor.Key);
                if (npc != null)
                {
            ...
            // 선택지 기록
            _state.AddChoice(choice.Id);
            
            // 이벤트 완료 처리
            if (gameEvent.IsOneTime)
            {
                _state.CompleteEvent(gameEvent.Id);
            }
